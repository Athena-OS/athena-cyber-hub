...
logger = logging.getLogger("Athena:Container")


@Gtk.Template(resource_path='/org/athenaos/CyberHub/gtk/container.ui')
class AthenaAchContainer(Adw.ActionRow):
    __gtype_name__ = "AthenaAchContainer"
    btn_shell = Gtk.Template.Child()
    btn_init = Gtk.Template.Child()
    btn_delete = Gtk.Template.Child()
    btn_readme = Gtk.Template.Child()
    ...
    def __build_ui(self):
        ...
        self.btn_delete.set_visible(self.__container["Status"] == 0)
        ...
        self.btn_readme.connect("clicked", self.__on_btn_readme_clicked)
    ...
    def __on_btn_delete_clicked(self, widget):
        res = GLib.spawn_command_line_async(self.__container["DeleteCmd"])
        if res:
            self.btn_init.set_visible(True)
            self.btn_shell.set_visible(False)
            self.btn_delete.set_visible(False)
            self.__window.toast(_("Shell deleted for container '{}'").format(self.__container["Name"]))

    def __on_btn_readme_clicked(self, widget):
        GLib.spawn_command_line_async(self.__container["ReadCmd"])
        self.__window.toast(_("Read document for container '{}'").format(self.__container["Name"]))
